...
<c:if test="${ action ne null && keyword ne null  }" >
<div style="text-align:center;">
	<%-- <% if(currentPage <= 1){ %> --%>
...
<c:if test="${ action != null && action eq 'date' }">
<div style="text-align:center;">
	<%-- <% if(currentPage <= 1){ %> --%>
	<c:if test="${ currentPage le 1 }">
		[맨처음] &nbsp;
...
<%-- 	<% for(int p = startPage; p <= endPage; p++){ 
	...
	%> --%>
	<c:forEach begin="${ startPage }" end="${ endPage }" step="1" var="p">
		<c:if test="${ p eq currentPage }"></c:if>
			<font color="blue" size="4"><b>${ p }</b></font>
	<%-- <%     }else{ %> --%>
		<c:if test="${ p ne currentPage }">
	...
	<%-- 다음 페이지 그룹으로 이동 --%>
	<%-- <% if((currentPage + 10) > endPage && (currentPage + 10) < maxPage){  //다음그룹이 있다면 %> --%>
	<c:if test="${ (currentPage + 10) gt endPage && (currentPage + 10) lt maxPage  }">
		<a href="/first/${ urlMapping }?page=${ startPage + 10 }">[다음그룹]</a> &nbsp;
	</c:if>	
	<%-- <% }else{ //다음그룹이 없다면 %> --%>
	<c:if test="${ !((currentPage + 10) gt endPage && (currentPage + 10) lt maxPage)  }">
