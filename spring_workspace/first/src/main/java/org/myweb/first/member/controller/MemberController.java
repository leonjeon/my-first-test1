...
public class MemberController {
	...
	//스프링에서는 부모 인터페이스 타입으로 래퍼런스 선언함 (다형성 이용함)
	//실행시 후손의 오버라이딩된 메소드를 연결 실행하게 됨 (동적 바인딩 이라고 한다.)
	 ...
	 * model) { logger.info("login.do run()....");
	 * 
	 * //1. 전송온 값 꺼내서 객체에 저장하기 Member member = new Member();
	 * member.setUserId(request.getParameter("userid"));
	 * member.setUserPwd(request.getParameter("userpwd"));
	 * 
	 * //2. 서비스 메서드로 객체 전달하고 실행된 결과받기 Member loginMember =
	 ...
	 * { //스프링에서는 request 에 저장처리하는 내용(객체정보)은 스프링이 제공하는 Model 을 사용함 //포워딩 못 함 =>
	 * viewResolver로 ViewFile명과 View로 내보낼 값이 전달되는 구조이다.
	 * model.addAttribute("message", "로그인 실패유! 아이디나 암호를 다시 확인하셔유~!"); return
	 * "common/error"; } }
	 */
	
	//로그인 처리용 메서드 : command 객체 사용
	//서버로 전송온 parameter 값을 저장하는 객체를 command 객체라고 한다.
	...
	public String loginMethod(Member member, HttpSession session, SessionStatus status, Model model) {
		...
		logger.info("아이디 : " + member.getUserId() + " " + member.getUserId().length());
		//서비스 메서드로 보내고 결과받기
		...
		if(loginMember != null &&
			...
			model.addAttribute("message", "로그인 실패유~! 아이디나 암호를 확인해봐유~! 또는 로그인 제한된 회원이유... 관리자에게 문의하슈");
			return "common/error";
		}
	}
	...
	public String logoutMethod(HttpServletRequest request, Model model) {
		...
		if(session != null) {
			...
			model.addAttribute("message", "로그인 세션이 존재하지 않아유!!");
			return "common/error";
		}
	}
	...
	public String memberInsertMethod(Member member, Model model) {
		...
		if(memberService.insertMember(member) > 0) {
			...
			model.addAttribute("message", "회원 가입 실패!! 꺼지셈");	
	...
	public void dupCheckIdMethod(@RequestParam("userid") String userId, 
		...
		//메서드 매개변수 영역에서 사용하는 어노테이션 중에
		//@RequestParam("전송온이름") 자료형 값저장변수명
		//자료형 값 저장 변수명 = request.getParameter("전송온이름"); 코드와 같음
	...
	//MyPage 클릭 시 내 정보보기 요청 처리용 메소드
	// 컨트롤러에서 뷰리졸버로 리턴하는 타입은 String, ModelAndView 를 사용할 수 있다.
	@RequestMapping("myinfo.do")
	public String memberDetailMethod(@RequestParam("userId") String userId, Model model) {
		//서비스 메서드로 아이디 전달하고, 해당 회원정보 받기
	...
	//회원 정보 수정페이지로 이동 처리용 메서드 <= 수정할 회원정보도 함께 보내야 함
	...
	//회원정보 수정 처리용 메서드 : 수정 성공시 main.jsp 로 이동함
	...
	public String memberUpdateMethod(Member member, Model model,
		...
		logger.info("보내기 전 : " + member);
		if(memberService.updateMember(member) > 0) {
			//뷰리졸버로 리턴하지 않고, 바로 컨트롤러의 다른 메서드를 실행할 경우
			return "redirect:main.do";
			//만약, 회원정보 수정 성공시 마이페이지로 이동하게 한다면
	...
	public String memberDeleteMethod(@RequestParam("userId") String userId, Model model) {
		...
		if(memberService.deleteMember(userId) > 0) {
			//컨트롤러 또는 다른 컨트롤러의 메서드를 직접 호출할 경우
	...
	/*
	 * @RequestMapping("mlist.do") public String memberListViewMethod(Model model) {
...
//	//관리자용 : 회원 검색 처리용 메서드 : 페이징 처리 없는 코드
...
//	}
	...
	//관리자용 : 회원 검색 처리용 메서드 : 페이징 처리 있는 코드
		@RequestMapping(value="msearch.do", method=RequestMethod.POST)
		public ModelAndView memberSearchMethod(HttpServletRequest request, ModelAndView mv) {
			//전송온 값 꺼내기
			String action = request.getParameter("action");
			...
			//총 페이지수 계산을 위해 검색 결과가 적용된 총 목록 갯수를 조회
			int listCount = 0;
			switch(action) {
