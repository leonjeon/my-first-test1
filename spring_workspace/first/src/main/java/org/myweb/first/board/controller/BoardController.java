...
import org.myweb.first.board.model.survice.BoardService;
...
public class BoardController {
	...
	public String moveBoardUpdatePage(
		...
		if(board != null) {
			...
			model.addAttribute("message", board + "번 게시글 수정페이지로 이동 실패!!");
			return "common/error";
		}
	}
	...
	//첨부파일 다운로드 요청 처리용
	//파일 다운로드 처리용 메소드는 리턴 타입 ModelAndView 로 정해져 있음
	...
	public ModelAndView fileDownMethod(
	  ...
	  public String replyInsertMethod(Board reply,
		  ...
		  // 대댓글(댓글의 댓글)일 때는 boardReplyRef (참조 댓글번호) 지정
	...
	public String boardDetailMethod(@RequestParam("bnum") int boardNum,
	...
	public String boardInsertMethod(Board board, Model model, HttpServletRequest request,@RequestParam(name="upfile", required=false) MultipartFile mfile) {
	...
	public String boardTop3Method() throws UnsupportedEncodingException {
		...
		//1. 출력스트림으로 응답하는 방법
		//2. 뷰리졸버로 리턴하는 방법 : response body에 내보낼 값을 지정함 (현재 우리가 사용하는 방법)
		...
		for(Board board : list) {
			...
			//한글에 대해선 인코딩해서 json에 담아야함 (한글 깨짐 방지)
		...
		//전송용 json을 json string으로 바꿔서 전송되게 함
	...
	public String boardListMethod(
			@RequestParam(name="page", required=false) String page,  //페이지가 누락되어도 에러 x 처리
			@RequestParam(name="limit", required=false) String slimit,
			Model model) {
		int currentPage = 1;
		if(page != null) { //null 이면 1페이지, 아니면 해당 페이지 출력함
			currentPage = Integer.parseInt(page);
		}
		...
		int limit = 10;
		if(slimit != null) {
			limit = Integer.parseInt(slimit); //전송받은 한 페이지에 출력할 목록 수를 적용
		}
		
		//총 페이지수 계산을 위해 게시글 전체 수를 조회해 옴
